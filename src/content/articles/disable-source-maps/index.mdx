...
I'm pretty sure **source maps should not be enabled by default in developer tool debuggers**. Browsers break many critical features of their debuggers, by default, as soon as source maps are available, and break them in a way that is totally not obvious to the developer (since rendering `undefined` in a tooltip might just make it seem like the value really is `undefined` instead of the actual issue, which is that you are checking the right expression).
